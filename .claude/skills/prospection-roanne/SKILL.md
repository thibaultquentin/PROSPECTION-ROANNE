...
- Ajouter une ligne dans le Google Sheets **SUIVI PROSPECTION** via le webhook Apps Script (POST JSON, body brut — pas de form-urlencoded, le script fait un `JSON.parse` direct qui plante sinon) :
  `https://script.google.com/macros/s/AKfycbydrO4qczgwdeAP8Z2NGE73L6LWQvLyt1IrmNpWW2hKxG6xepQ4h2PHVa_9UitGYxxn/exec`
  Payload : `{"action":"upsert","entreprise":"...","ville":"...","site":"...","email":"...","statut":"Envoyé","dossierDrive":"...","lien":"URL GitHub Pages"}` (clés en minuscules, obligatoire — une clé mal casée comme `Entreprise` est silencieusement ignorée et laisse la cellule vide). Une réponse `{"status":"ok"}` confirme l'écriture.
  Le champ `date` n'a pas besoin d'être envoyé : le script tamponne toujours la date du jour à l'écriture, quoi qu'on lui passe.
  **Limite connue (vérifiée empiriquement, pas corrigée à ce jour) : le webhook fait un simple append, jamais une vraie mise à jour ni une suppression.** Envoyer deux fois la même entreprise crée deux lignes, pas une correction ; `{"action":"delete",...}` ajoute une ligne au lieu d'en retirer une. Conséquence pratique : **appeler ce webhook une seule fois par entreprise, jamais en retry automatique** — si la requête échoue (erreur réseau, pas de `{"status":"ok"}` en retour), le signaler à l'utilisateur plutôt que de renvoyer la requête à l'aveugle, et ne jamais s'en servir pour nettoyer une ligne existante (doublon, ligne test) : ça revient à l'utilisateur de le faire directement dans le Sheets.
  Si le webhook est injoignable ou renvoie une erreur, revenir au filet de sécurité : donner à l'utilisateur, en clair dans la conversation, la ligne prête à copier-coller (Entreprise, Ville, Site, Email, Statut, Dossier Drive, Lien maquette) et signaler ce geste comme le seul reste manuel de l'étape 7.
